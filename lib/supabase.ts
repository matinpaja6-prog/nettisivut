...
const VEHICLE_LISTING_COUNT_TYPES: Record<keyof VehicleListingCounts, string[]> = {
  Moottoripyörä: ["Moottoripyörä", "Moottoripyörät"],
...
export async function likeSellerReview(reviewId: string) {
  ...
  try {
    ...
    if (!user) {
      return {
        error: new Error("Kirjaudu sisään tykätäksesi arvostelusta.")
      };
    }
...
export async function unlikeSellerReview(reviewId: string) {
  ...
  try {
    ...
    if (!user) {
      return {
        error: new Error("Kirjaudu sisään hallitaksesi peukkuja.")
      };
    }
