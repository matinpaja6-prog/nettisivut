...
export default function CartPage() {
  ...
  if (loading) return <main className={`${styles.page} ${styles.cartWizardPage}`}><div className={styles.cartLoading}><ShoppingBag size={30} /><span>Ladataan ostoskoria…</span></div></main>;
  if (!cart.length) return <main className={`${styles.page} ${styles.cartWizardPage} ${styles.cartEmptyPage}`}><div className={styles.cartEmptyBare}><ShoppingCart size={52} strokeWidth={2.2} /><h1>Ostoskorisi on tyhjä</h1>{cartNotice && <p className={styles.warning}>{cartNotice}</p>}<Link href="/ilmoitukset">Jatka ostoksia</Link></div></main>;
    ...
    <main className={`${styles.page} ${styles.cartWizardPage}`}>
      <div className={`${styles.shell} ${styles.cartWizardShell}`}>
        <header className={styles.cartWizardHero}>
          <div>
            <Link className={styles.cartBackLink} href={company ? profilePath(company.owner_user_id, company.name, "fi") : "/ilmoitukset"}><ArrowLeft size={16} /> Jatka ostoksia</Link>
            <div className={styles.eyebrow}>Maskines turvallinen kassa</div>
          ...
          <div className={styles.cartTrustRow}><span><ShieldCheck size={16} /> Vahvistettu myyjä</span><span><LockKeyhole size={16} /> Suojattu maksu</span></div>
        </header>
        ...
        <div className={styles.cartLayout}>
            ...
            {step === 1 && <section className={`${styles.panel} ${styles.checkoutStage}`}>
              ...
              <div className={styles.cartSellerGroups}>{sellerGroups.map((group) => <section className={styles.cartSellerGroup} key={group.companyId}>
                ...
                <div className={styles.cartLines}>{group.lines.map(({ product, quantity }) => { const unitPrice = activeSalePrice(product); return <article className={styles.cartLine} key={product.id}>
                  ...
                  <div className={styles.cartLineInfo}><div><Link href={`/tuotteet/${product.id}`}>{product.name}</Link><p>{unitPrice < product.price_cents && <del>{money(product.price_cents)}</del>} {money(unitPrice)} / kpl</p></div><div className={styles.cartDeliveryTags}>{productSupportsPickup(product) && <span><MapPin size={13} /> Nouto</span>}{productSupportsPosti(product) && <span><Truck size={13} /> Posti</span>}</div></div>
                  <div className={`${styles.cartLineActions} ${product.stock_quantity <= 1 ? styles.cartLineActionsSingle : ""}`}>{product.stock_quantity > 1 && <div className={`${styles.quantityStepper} ${quantity >= product.stock_quantity ? styles.quantityStepperMax : ""}`} aria-label={`Tuotteen ${product.name} määrä`}><button type="button" aria-label="Vähennä määrää" onClick={() => setQuantity(product.id, quantity - 1)}><Minus size={15} /></button><input type="number" min={1} max={product.stock_quantity} value={quantity} aria-label="Määrä" onChange={(event) => setQuantity(product.id, Number(event.target.value))} />{quantity < product.stock_quantity && <button type="button" aria-label="Lisää määrää" onClick={() => setQuantity(product.id, quantity + 1)}><Plus size={15} /></button>}</div>}<strong>{money(unitPrice * quantity)}</strong><button type="button" className={styles.cartRemove} onClick={() => remove(product.id)}><Trash2 size={15} /> Poista</button></div>
                </article>; })}</div>
                <div className={styles.sellerCoupon}><div><strong>Onko sinulla yrityksen alekoodi?</strong><small>Koodi koskee vain yrityksen {group.company.name} tuotteita.</small></div><form onSubmit={(event) => { event.preventDefault(); void applyCoupon(group.companyId, group.productTotal); }}><input value={sellerChoices[group.companyId]?.couponInput ?? ""} onChange={(event) => updateSellerChoice(group.companyId, { couponInput: event.target.value, couponCode: "", couponDiscount: 0 })} placeholder="ALEKOODI" /><button type="submit">Käytä</button></form>{Boolean(sellerChoices[group.companyId]?.couponDiscount) && <span className={styles.couponSuccess}><Check size={15} /> {sellerChoices[group.companyId].couponCode}: −{money(sellerChoices[group.companyId].couponDiscount)}</span>}</div>
              </section>)}</div>
            </section>}

            {step === 2 && <section className={`${styles.panel} ${styles.checkoutStage}`}>
              <div className={styles.checkoutStageHeader}><span><UserRound size={22} /></span><div><div className={styles.eyebrow}>Vaihe 2 / 4</div><h2>Ostajan tiedot</h2><p>Kuitti, tilausvahvistus ja toimitusviestit lähetetään näillä tiedoilla.</p></div></div>
              {authChecked && !signedIn && <div className={styles.checkoutBuyerType}>
                <div className={styles.checkoutBuyerTypeHeading}><strong>Kuka tekee tilauksen?</strong><p>Yksityishenkilö voi tilata ilman tiliä. Yritysosto vaatii yritystilin.</p></div>
                <div className={styles.checkoutBuyerTypeGrid}>
                  <button type="button" className={buyerType === "private" ? styles.checkoutBuyerTypeActive : styles.checkoutBuyerTypeOption} onClick={() => setBuyerType("private")}><span><UserRound size={21} /></span><div><strong>Yksityishenkilö</strong><small>Jatka ilman käyttäjätiliä</small></div>{buyerType === "private" && <Check size={17} />}</button>
                  <button type="button" className={buyerType === "company" ? styles.checkoutBuyerTypeActive : styles.checkoutBuyerTypeOption} onClick={() => setBuyerType("company")}><span><Building2 size={21} /></span><div><strong>Yritys</strong><small>Kirjaudu tai luo yritystili</small></div>{buyerType === "company" && <Check size={17} />}</button>
                </div>
              </div>}
              {authChecked && signedIn && <div className={styles.checkoutIdentityReady}><Check size={18} /><div><strong>{buyerType === "company" ? "Tunnistettu yritysostajaksi" : "Tunnistettu yksityisasiakkaaksi"}</strong><p>Ostajatyyppi ja yhteystiedot haettiin automaattisesti tililtäsi.</p></div></div>}
              {authChecked && !signedIn && buyerType === "company" && <div className={styles.checkoutCompanyRequired}><Building2 size={22} /><div><strong>Yritysosto vaatii yritystilin</strong><p>Luo yritystili tai kirjaudu olemassa olevalle yritystilille. Yrityksen tiedot tunnistetaan sen jälkeen automaattisesti.</p></div><div><Link href={`/auth?mode=register&account=company&next=${encodeURIComponent("/ostoskori")}`}>Luo yritystili</Link><Link href={`/auth?next=${encodeURIComponent("/ostoskori")}`}>Kirjaudu</Link></div></div>}
              {authChecked && !signedIn && buyerType === "private" && <div className={styles.checkoutIdentityGuest}><UserRound size={19} /><div><strong>Voit jatkaa ilman Maskines-tiliä</strong><p>Halutessasi voit kirjautua, jolloin tallennetut tietosi täytetään automaattisesti.</p></div><Link href={`/auth?next=${encodeURIComponent("/ostoskori")}`}>Kirjaudu</Link></div>}
              ...
              {buyerType === "company" && <div className={styles.checkoutFormSection}>
                <div className={styles.checkoutFormTitle}><Building2 size={17} /><div><strong>Yrityksen tiedot</strong><small>Tiedot haetaan vahvistetulta yritystililtä</small></div></div>
                <div className={styles.formGrid}>
                  <label className={styles.field}><span>Yrityksen nimi *</span><input autoComplete="organization" value={customerCompany} readOnly aria-readonly="true" /></label>
                  <label className={styles.field}><span>Y-tunnus *</span><input value={customerBusinessId} readOnly aria-readonly="true" /></label>
                </div>
                <div className={styles.checkoutCompanySource}><LockKeyhole size={14} /><span>Yritystiedot on lukittu tälle tilille.</span><Link href="/profile#yritys">Muokkaa yritystilillä</Link></div>
              </div>}
              ...
              <div className={styles.checkoutFormSection}>
                ...
                <div className={styles.formGrid}>
                  <label className={styles.fieldFull}><span>Katuosoite *</span><input autoComplete="street-address" value={customerAddress} onChange={(event) => { setCustomerAddress(event.target.value); setPoints([]); setSellerPickupPoints({}); }} placeholder="Esimerkkikatu 12 A 4" /></label>
                  <label className={styles.field}><span>Postinumero *</span><input autoComplete="postal-code" inputMode="numeric" maxLength={countrySettings(customerCountry).postalLength} value={postalCode} onChange={(event) => { setPostalCode(event.target.value.replace(/\D/g, "").slice(0, countrySettings(customerCountry).postalLength)); setPoints([]); setSellerPickupPoints({}); }} placeholder={countrySettings(customerCountry).postalPlaceholder} /></label>
                  <label className={styles.field}><span>Kaupunki *</span><input autoComplete="address-level2" value={customerCity} onChange={(event) => { setCustomerCity(event.target.value); setPoints([]); setSellerPickupPoints({}); }} placeholder={countrySettings(customerCountry).cityPlaceholder} /></label>
                  <label className={styles.fieldFull}><span>Maa *</span><select autoComplete="country" value={customerCountry} onChange={(event) => { setCustomerCountry(event.target.value); setPostalCode(""); setPoints([]); setSellerPickupPoints({}); setPickupSearchQuery(""); setPickupSearchArea(""); }}>{NORDIC_COUNTRIES.map((country) => <option value={country.code} key={country.code}>{country.name}</option>)}</select></label>
                </div>
              </div>
              </> : null}
            </section>}
              ...
              <div className={styles.checkoutPaymentReview}>
                ...
                <div className={styles.maskinesPaymentShell}>
                  ...
                  {!paymentClientSecret && <div className={styles.paymentMethodsLoading} role="status" aria-live="polite">
                    ...
                    {!submitting && <button type="button" onClick={() => void checkout()}>Näytä maksutavat</button>}
                  </div>}
          ...
          <aside className={`${styles.panel} ${styles.cartSummary}`}>
            <div className={styles.cartSummarySeller}><span>Myyjät</span><strong>{sellerGroups.length} vahvistettua yritystä</strong><small>Jokaiselle yritykselle muodostuu oma alitilaus</small></div>
            <h2>Tilauksen yhteenveto</h2>
            <div className={styles.cartTotals}><p><span>Tuotteet ({itemCount})</span><strong>{money(productTotal)}</strong></p>{couponTotal > 0 && <p className={styles.cartDiscountTotal}><span>Alennuskoodit</span><strong>−{money(couponTotal)}</strong></p>}<p><span>Toimitukset ({sellerGroups.length})</span><strong>{shippingPrice === 0 ? "Maksuton" : money(shippingPrice)}</strong></p>{sellerTotals.map((group) => <p className={styles.cartSellerTotal} key={group.companyId}><span>{group.company.name}</span><strong>{money(group.total)}</strong></p>)}</div>
            <div className={styles.cartGrandTotal}><span>Maksettavaa</span><strong>{money(productTotal - couponTotal + shippingPrice)}</strong><small>Yksi maksu, yrityskohtaiset tilaukset</small></div>
            ...
            <div className={styles.checkoutStageActions}>
              {step > 1 && !paymentClientSecret && <button type="button" className={styles.checkoutBackButton} onClick={() => openStep((step - 1) as CheckoutStep)}><ArrowLeft size={17} /> Edellinen</button>}
              ...
              {step === 4 && !paymentClientSecret && submitting && <div className={styles.paymentReadyBadge}><LockKeyhole size={18} /><span><strong>Avataan maksutapoja…</strong><small>Maksunäkymä latautuu automaattisesti.</small></span></div>}
              ...
              {step === 4 && paymentClientSecret && <div className={styles.paymentReadyBadge}><CircleCheck size={18} /><span><strong>Maksuvaihe avattu</strong><small>Viimeistele maksu vasemmalla.</small></span></div>}
            </div>
            <div className={styles.cartSecurity}><span><Check size={15} /> Hinta ja varastosaldo tarkistetaan</span><span><Check size={15} /> Kuitti lähetetään maksun jälkeen</span><span><Check size={15} /> Turvallinen Stripe-maksu</span></div>
            <button className={styles.clearCartButton} onClick={() => { clearCart(); setCart([]); }}><Trash2 size={14} /> Tyhjennä ostoskori</button>
          </aside>
        </div>
      </div>
    </main>
